...
public interface ClientTfseCommentService {
...
@Service
@AllArgsConstructor
class ClientTfseCommentServiceImpl implements ClientTfseCommentService {
...
public Page<TfseCommentDto.Detail> list(Pageable pageable, TfseCommentSearch tfseCommentSearch, @AuthenticationPrincipal UserDetails userDetail) {
    ...
    return tfseCommentRepository.findAll(tfseCommentSearch.search(), pageable).map(data -> TfseCommentMapper.INSTANCE.entityToDetailDto(data, user.getUid()));
}
