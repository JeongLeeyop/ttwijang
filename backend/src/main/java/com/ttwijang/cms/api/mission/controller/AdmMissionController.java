...
import com.ttwijang.cms.api.mission.service.AdmMissionService;
...
@AllArgsConstructor
public class AdmMissionController {
    private final AdmMissionService admMissionService;
    ...
    public ResponseEntity<MissionDto.detail> detail(@AuthenticationPrincipal SinghaUser authUser,@PathVariable("uid") String uid) {
        return ResponseEntity.ok(admMissionService.detail(authUser, uid));
    }

    @GetMapping
    public ResponseEntity<Page<MissionDto.detail>> list(@AuthenticationPrincipal SinghaUser authUser, MissionSearch search, @PageableDefault(direction = Direction.DESC, sort = { "createDate" }) Pageable pageable) {
        return ResponseEntity.ok(admMissionService.list(authUser, search, pageable));
    }
    
    @PostMapping
    public ResponseEntity add(@AuthenticationPrincipal SinghaUser authUser, @RequestBody MissionDto.add addDto) {
        admMissionService.add(authUser, addDto);
        return ResponseEntity.ok().build();
    }
    
    @DeleteMapping("{uid}")
    public ResponseEntity delete(@AuthenticationPrincipal SinghaUser authUser, @PathVariable("uid") Mission mission) {
        admMissionService.delete(authUser, mission);
        return ResponseEntity.ok().build();
    }
    
    @PutMapping("{uid}")
    public ResponseEntity update(@AuthenticationPrincipal SinghaUser authUser, @PathVariable("uid") Mission mission,@RequestBody MissionDto.update updateDto) {
        admMissionService.update(authUser, mission, updateDto);
        return ResponseEntity.ok().build();
    }
    ...
    //     admMissionService.updateSecretStatus(authUser, updateDto);
    //     return ResponseEntity.ok().build();
    // }
}
