...
export default class extends Vue {
  ...
  private handleClickBefore() {
    const arr = ['FoodSearch', 'FoodRecent', 'FoodDirect', 'FoodHistory'];
    const arr2 = ['BodyRecord', 'BodyPurpose'];

    if (this.$route.name === 'CommunityDetail') {
      this.$router.push({ name: 'Community' });
    } else if (this.$route.name === 'Community') {
      this.$router.push({ name: 'Home' });
    } else if (arr.includes(this.$route.name ? this.$route.name : '')) {
      this.$router.push({ name: 'Diary', query: { tab: 'diary', diaryDate: this.$route.query.diaryDate } });
    } else if (arr2.includes(this.$route.name ? this.$route.name : '')) {
      this.$router.push({ name: 'Diary', query: { tab: 'body', diaryDate: this.$route.query.diaryDate } });
    } else {
      this.$router.go(-1);
    }
  }
