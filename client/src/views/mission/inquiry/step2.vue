...
export default class extends Vue {
  mounted() {
    ...
    MissionInquiryModule.setPageClean(2);
  ...
  private getMissionInquiryList() {
    getMissionInquiryList(2).then((res) => {
      if (res.data) {
        // 페이지 제목 설정
  ...
  private handleUpdateUserInfo() {
    ...
    MissionInquiryModule.setPage({ page: 2, content: data });
    this.$router.push({ name: 'MissionInquiry3' });
  }
}
</script>
