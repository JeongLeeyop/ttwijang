...
public class ClientTfseController {
    ...
    @GetMapping
    public ResponseEntity<List<TfseDto.list>> list(@AuthenticationPrincipal SinghaUser authUser, TfseSearch search) {
        return ResponseEntity.ok(TfseService.list(authUser, search));
    }
    
    @GetMapping("community")
    public ResponseEntity<Page<TfseDto.list>> communityList(@AuthenticationPrincipal SinghaUser authUser, TfseSearch search, @PageableDefault(
					size=10,
					page=0,
